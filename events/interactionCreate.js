  ...
  async execute(interaction) {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('trade_decline_')) {
        const tradeId = interaction.customId.slice('trade_decline_'.length);
        const trade = getTrade(tradeId);
        ...
        const originalMsg = await interaction.channel.messages.fetch(trade.messageId).catch(() => null);
        if (originalMsg) {
      ...
      if (interaction.customId.startsWith('trade_confirm_')) {
        const tradeId = interaction.customId.slice('trade_confirm_'.length);
        const trade = getTrade(tradeId);
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith('trade_offer_')) {
        const tradeId = interaction.customId.slice('trade_offer_'.length);
        const trade = getTrade(tradeId);
        ...
        if (originalMsg) {
          ...
          const tradeSummary = `**Trade Summary**\n${initiatorUser.tag} offers: **${trade.initiatorOffer}** coins\n${targetUser.tag} offers: **${trade.targetOffer}** coins\n\nBoth users must click **Confirm Trade** below.`;
