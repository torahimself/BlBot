...
const { addMemberToRole, getRoleOwner } = require('../../utils/economy/shopManager.js');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    db.get('SELECT roleId FROM purchased_roles WHERE ownerId = ?', [userId], async (err, row) => {
      ...
      const result = await addMemberToRole(interaction, row.roleId, targetUser);
