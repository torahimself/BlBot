...
// Identical to /jail in every way except the role applied — this uses
// config.jail.jailpRoleId instead of config.jail.jailRoleId. The auto
// scanner, rejoin protection, and /unjail all work the same for both.
module.exports = {
    data: new SlashCommandBuilder()
        .setName('jailp')
        .setDescription('Jail a user with the jailp role — strips their roles and applies it (admin/staff only)')
    ...
    async execute(interaction) {
        ...
        const result = await jailUser(
            interaction.client, guild, member, interaction.user, reason, durationMs, evidence,
            config.jail.jailpRoleId
        );
        ...
        await interaction.editReply(`✅ <@${targetUser.id}> has been jailed (jailp). Auto-unjail: ${durationText}. Saved ${result.previousRoleCount} previous role(s).`);
    }
};
